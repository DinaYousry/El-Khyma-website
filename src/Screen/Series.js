...
const Series = () => {
  ...
  const callSingleSerie = ( name , image , brief ,video ,time , crew , id) => {
    navigation(`/serieselement/?name=${name}&image=${image}&brief=${brief}&video=${video}&time=${time}&crew=${crew}&id=${id}`);
            ...
            fetch(`http://hwayadesigns-001-site2.itempurl.com/api/Series/List?pageNo=${currentPageNum}&pageSize=${pageSize}`)
            .then((response) => {
    ...
    const alyGetMeals = async (page) => {
      fetch(
        `http://hwayadesigns-001-site2.itempurl.com/api/Series/List?pageNo=${page}&pageSize=${pageSize}`
      )
        .then((response) => {
          console.log(response);
          return response.json();
    ...
    async function search(key) {
      ...
      let result = await fetch(
        `http://hwayadesigns-001-site2.itempurl.com/api/Series/ListSearch?pageNo=1&pageSize=10&search=${key}`
      );
  ...
  return (
    <>
         ...
         <Row>
           ...
           <Col lg={6} xs={4}>
           <br/>
           <h3 style={{color:"#df403e" , textAlign:"right" , paddingRight:"10%"}}>  تشوف ايه ؟</h3>
           </Col>
         </Row>
