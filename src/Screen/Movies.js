...
const Movies = () => {
  ...
  useEffect(() => {
    const getMeals = async () => {
      fetch(
        `http://hwayadesigns-001-site8.ftempurl.com/api/movies?page=${currentPageNum}&pageSize=${pageSize}`
      )
        .then((response) => {
  ...
  const alyGetMeals = async (page) => {
    fetch(
      `http://hwayadesigns-001-site8.ftempurl.com/api/movies?page=${page}&pageSize=${pageSize}`
    )
      .then((response) => {
        console.log(response);
        return response.json();
    ...
    <>
      <div className="no">
        <Nav className="justify-content-center ">
          <Nav.Item>
            <NavLink className="navii" to="/plays">
              مسرحيات{" "}
            </NavLink>
          </Nav.Item>
          <Nav.Item>
            <NavLink className="navii" to="/seriesmedia">
              {" "}
              مسلسلات
            </NavLink>
          </Nav.Item>
          <Nav.Item>
            <NavLink className="navii" to="/movies">
              {" "}
              افلام
            </NavLink>
          </Nav.Item>
      ...
      <br />
      <h3 style={{ color: "#981916" }}>افلام </h3>
