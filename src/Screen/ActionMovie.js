...
const ActionMovie = () => {
  ...
  useEffect(() => {
    const getMeals = async () => {
      fetch(
        `http://hwayadesigns-001-site8.ftempurl.com/api/movies/category?page=${currentPageNum}&pageSize=${pageSize}&Type=2`
      )
        .then((response) => {
  ...
  const alyGetMeals = async (page) => {
    fetch(
      `http://hwayadesigns-001-site8.ftempurl.com/api/movies/category?page=${page}&pageSize=${pageSize}&Type=2`
    )
      .then((response) => {
        console.log(response);
        return response.json();
    ...
    <>
     ...
     <div className='no'>
      ...
      <br/>
        ...
        <br/>
      <h3 style={{color :"#981916"}}>افلام اكشن </h3>
