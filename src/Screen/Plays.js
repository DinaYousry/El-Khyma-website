...
// import { series } from "../Assets/Index";
...
const Plays = () => {
  ...
  const callSingleProduct = (youtubeUrl, ar_name) => {
    navigation(`/movie/?youtubeUrl=${youtubeUrl}&ar_name=${ar_name}`);
            ...
            fetch(`http://hwayadesigns-001-site8.ftempurl.com/api/Plays?page=${currentPageNum}&pageSize=${pageSize}`)
            .then((response) => {
...
const alyGetMeals = async (page) => {
  fetch(
    `http://hwayadesigns-001-site8.ftempurl.com/api/Plays?page=${page}&pageSize=${pageSize}`
  )
    .then((response) => {
      console.log(response);
      return response.json();
